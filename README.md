...
| First DirecTV tune after reboot opens the app but not the channel, or tunes YES/continue-watching instead of the requested channel | Cold start drops the VIEW intent, or leftover MediaSession PLAYING is treated as ready | Update **server to 0.1.26** and **Agent to 0.1.25**. Grant Notification access. **Update Agent** on each Streamer so deeplinks use CLEAR_TOP. Optional: Options → **Deeplink relaunch** |
