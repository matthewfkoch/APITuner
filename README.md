...
| First DirecTV tune after reboot hangs on the splash screen then fails | DirecTV splash ignores VIEW intents and never reports MediaSession PLAYING; 0.1.27 retried twice then timed out at 30s and dropped the HDMI proxy | Update **server to 0.1.28** (Agent **0.1.27** is fine). Grant Notification access. First-boot HOMEs off the splash, retries twice with `CLEAR_TASK`, then waits another 45s for playback. Optional: Options → **Deeplink relaunch**; pair **Keys / D-pad** if splash still needs an OK press |
