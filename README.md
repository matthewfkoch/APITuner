...
| Tune ends on the Android TV home screen; logs say `HTTP verb {}POST unhandled` | 0.1.28 sends HOME (`POST /api/stop` with body `{}`) before a DirecTV relaunch. Agent 0.1.27 does not read that body, so the follow-up deeplink never starts | Update the **server to 0.1.29**. Agent **0.1.27** is enough for this fix. Agent **0.1.29** also drains unread POST bodies. Notification access is already enough for playback detection |
| First DirecTV tune after reboot hangs on the splash screen then fails | DirecTV splash ignores VIEW intents and never reports MediaSession PLAYING | Update the server. Grant Notification access. The server HOMEs off a stuck splash, retries with `CLEAR_TASK`, then waits another 45s. Pair **Keys / D-pad** if the splash still needs an OK press |
| Grant permissions (ADB) errors even though the Agent UI shows the permissions granted | Dashboard retried network ADB on port 5555 and did not trust the Agent's own permission report. Google TV Streamer often has no listener on 5555 | Update the server and click the button again. Overlay, usage, and notification already granted are treated as success. Accessibility (BACK/HOME/RECENTS only) is separate — enable it in the Agent app on Google TV |
| DirecTV tune shows video then drops after ~30s, or the second tuner “loses connection” | 0.1.25 treated program titles (Golden Girls) as the wrong channel vs call signs (MeTV) | Update **server to 0.1.26** (no Agent APK update) |
