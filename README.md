...
| First DirecTV tune after reboot opens the app but not the channel, or tunes YES/continue-watching instead of the requested channel | Cold start drops the VIEW intent, leftover PLAYING never changes, or splash `Unknown Title` | Update **server and Agent to 0.1.27**. Grant Notification access. First-boot retries the deeplink twice with `CLEAR_TASK`; idle cold starts retry after 2s. Optional: Options → **Deeplink relaunch** |
